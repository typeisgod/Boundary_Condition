function ret = LiuBC(img, pad_h,pad_v)
% input can be rgb
    [H, W, Ch] = size(img);
    H_w=2*pad_h;
    W_w=2*pad_v;
    img_size=[H+H_w,W+W_w];
...
%         ret(:,:,ch) = [C A C ;B img(:,:,ch) B; C A C];
ret(:,:,ch) = [ img(:,:,ch) B; A C];
    end
% 
ret = Shift_Img(ret,pad_h,pad_v);
end

function J=Shift_Img(I,pad_h,pad_w)
    [h,w,~]=size(I);
    J=[I(h-pad_h+1:h,:,:);I(1:h-pad_h,:,:)];
    J=[J(:,w-pad_w+1:w,:),J(:,1:w-pad_w,:)];
end
